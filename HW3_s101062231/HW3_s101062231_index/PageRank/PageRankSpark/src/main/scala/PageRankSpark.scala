...
object PageRankSpark {
  def main(args: Array[String]) {
    ...
    val lines = sc.textFile(filePath, sc.defaultParallelism * 10)
      ...
      var title = (pattern findAllIn line).mkString

          pattern = new Regex("<title>|</title>")
          title = (pattern replaceAllIn(title, "")).toString
         ...
         // title.replaceAll("&lt;", "<").replaceAll("&gt;", ">").replaceAll("&amp;", "&").replaceAll("&quot;", "\"").replaceAll("&apos;", "\'").capitalize
          ...
          .filter { _.length > 0 }
          outLink.map { link => (link, title) }.+:(title, "<title>") //map 回傳一個 Array(Array(link, title), Array(link, title), Array(link, title))
      })
    ...
    val danglingNum = infoList.map(x => x._2.length).filter { _ == 0 }.count();; 
    ...
    while (error > 0.001) {
      ...
      var tmpPageRankList = pageRankList.map(list => {
        list._2._2.map { outLinks => (outLinks, damping * list._2._1 / list._2._2.length) } //(link, a*(PR/linkCount))
                  .+:(list._1, (1 - damping)/totalNum + damping * danglingRankSum)          //多加一個(title, )
      ...
      pageRankList = tmpPageRankList.join(pageRankList.map(x => (x._1, x._2._2)), sc.defaultParallelism * 10)
      ...
      System.out.println("danglingNum: "+ danglingNum);
      System.out.println("**********************");
